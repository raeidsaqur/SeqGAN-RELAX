...
POS_NEG_SAMPLES = 10000 if not isDebug else 2000
...
if __name__ == '__main__':
    ...
    gen = generator.Generator(GEN_EMBEDDING_DIM, GEN_HIDDEN_DIM, VOCAB_SIZE, MAX_SEQ_LEN, gpu=CUDA, debug=isDebug)
    dis = discriminator.Discriminator(DIS_EMBEDDING_DIM, DIS_HIDDEN_DIM, VOCAB_SIZE, MAX_SEQ_LEN, gpu=CUDA, debug=isDebug)

    if CUDA:
        print("torch.cuda.is_available(): Using CUDA")
        oracle = oracle.cuda()
        gen = gen.cuda()
        dis = dis.cuda()
